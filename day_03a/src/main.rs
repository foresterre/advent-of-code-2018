...
fn solution(input: &str) -> u32 {
    ...
    let mut fabric: HashMap<Position, u32> = std::collections::HashMap::with_capacity(capacity);
    ...
    for line in lines {
        ...
        for x in x_from..x_to {
            for y in y_from..y_to {
                let get = fabric.get(&Position(x, y));

                match get {
                    Some(i) => fabric.insert(Position(x, y), i + 1),
                    None => fabric.insert(Position(x, y), 1),
                };
            }
        }
    }

    fabric.values().filter(|v| **v > 1).count() as u32
}
